...
class MArtikel extends CI_Model {
    ...
    public function get_artikel($id='',$where=[])
    {
        $this->see = "aiu.nama,a.*";
        $this->db->join('adm_info_usr aiu', 'aiu.id = a.ctdby', 'inner');
        $this->db->join('artikel_tag at', 'at.artikel_id = a.id', 'left');
    ...
    public function dt_artikel($aktif='',$date='')
    {
        ...
        foreach ($dataTabel as $dt) {
            ...
            $data[] = array(
                ...
                tgl_indo($dt->ctddate),
                $dt->nama,
                '<a href="'.site_url('adm/Artikel/edt_artikel/'.$dt->id).'"><button class="btn btn-default btn-sm text-uppercase font-weight-bold text-gray" style="padding:3px 10px !important;font-size: 13px;"><i class="fa fa-edit"></i></button></a>
                <a href="'.site_url('Blog/blog_single/'.$dt->id).'" target="_blank"><button class="btn btn-default btn-sm text-uppercase font-weight-bold text-gray" style="padding:3px 10px !important;font-size: 13px;"><i class="fa fa-eye"></i></button></a>
    ...
    public function de_artikel_gmb($id='')
    {
        $s = false;
        if (!empty($id) && $id != '') {
            $agmb = $this->db->get_where('artikel_gmb',['id' => $id]);
            if ($agmb->num_rows() > 0) {
                @unlink('data/artikel/'.$agmb->row()->img);
    ...
    public function de_artikel_id_gmb($id='')
    {
        $s = false;
        if (!empty($id) && $id != '') {
            $agmb = $this->db->get_where('artikel_gmb',['artikel_id' => $id]);
            if ($agmb->num_rows() > 0) {
                foreach ($agmb as $g) {
                    @unlink('data/artikel/'.$g->img);
                }
                $this->db->delete('artikel_gmb', ['artikel_id' => $id]);
                $q = $this->db->affected_rows();
                if ($q > 0) {
    ...
    public function up_artikel_tag($obj=[],$id_artikel='')
    {
        $s = false;
        if ($id_artikel != '') {
            $this->db->delete('artikel_tag',['artikel_id' => $id_artikel]);
            if (!empty($obj)) {
                $this->db->insert_batch('artikel_tag', $obj);
                $q = $this->db->affected_rows();
                if ($q > 0) {
    ...
    public function de_artikel_id_tag($id='')
    {
        $s = false;
        if (!empty($id) && $id != '') {
            $this->db->delete('artikel_tag', ['artikel_id' => $id]);
